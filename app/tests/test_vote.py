...
def test_vote(client_login):
    ...
    # Cast a vote
    vote_res = client_login.post("/vote/", json={"post_id": post_id, "dir": 1})
    ...
    # Remove the vote
    remove_vote_res = client_login.post("/vote/", json={"post_id": post_id, "dir": 0})
    assert remove_vote_res.status_code == 201
    assert remove_vote_res.json() == {
        "message": f"Successfully removed vote for post:{post_id}"
    }
